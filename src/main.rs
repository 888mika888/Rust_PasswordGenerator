...
fn main() {
    ...
    io::stdin().read_line(&mut copy_choice).expect("Failed to read line");

    if copy_choice.trim() == "y" {
            println!("Copy all or one? (all/one): ");
            let mut all_or_one = String::new();
            io::stdin().read_line(&mut all_or_one).expect("Failed to read line");
            ...
            if all_or_one.trim() == "all" {
                ...
                println!("WARNING: do NOT paste into a password field or browser address bar. All passwords will appear as one block of text.");
            } else if all_or_one.trim() == "one" {
                println!("Which one? (1-{}): ", count);
                let mut copy_input = String::new();
                io::stdin().read_line(&mut copy_input).expect("Failed to read line");
                let copy_index: usize = copy_input.trim().parse().expect("Please enter a valid number");
                clipboard.set_text(&passwords[copy_index - 1]).expect("Failed to copy");
                println!("Password {} copied to clipboard!", copy_index);
            }
        }
